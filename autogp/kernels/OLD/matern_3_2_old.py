...
class Matern_3_2(kernel.Kernel):
    ...
    def __init__(self, lengthscale=1.0, std_dev=1.0,
    ...
    def kernel(self, points1, points2=None):
        ...
        for i in range(kern_matrix.shape[0]):
            for j in range(kern_matrix.shape[1]):
                ...
                first_term = tf.scalar_mul(self.std_dev, 1.0 + tf.scalar_mul(math.sqrt(3.0)/self.lengthscale,distances))
                second_term = tf.exp(tf.scalar_mul(-math.sqrt(3.0)/self.lengthscale, distances))
